...
{
    public List<(int x, int y, bool IsEnemy)> GetValidMoves(int x, int y, IBoard board, string friendlyColor,
        Dictionary<PlayerColor, Dictionary<PieceType, bool>> movementStates, bool isCheck)
    {
        ...
        if (!isCheck) finalMoves.AddRange(GetValidCastlingMoves(x, y, board, friendlyColor, movementStates));
        return finalMoves;
    }

    public List<(int x, int y, bool IsEnemy)> GetAllValidMoves(IBoard board, string friendlyColor,
        Dictionary<PlayerColor, Dictionary<PieceType, bool>> movementStates, bool isCheck)
    {
        var friendlyPieces = board.GetPiecePositionsByColor(friendlyColor);
    ...
    private bool ValidateMove((int x, int y) from, (int x, int y) to, IBoard board, string currentTurn)
    {
        var fromPiece = board.Squares[from.x, from.y]!;
        var toPiece = board.Squares[to.x, to.y]!;
    ...
    private List<(int x, int y, bool IsEnemy)> GetValidCastlingMoves(int x, int y, IBoard board,
        string friendlyColor, Dictionary<PlayerColor, Dictionary<PieceType, bool>> movementStates)
    {
        var moves = new List<(int x, int y, bool IsEnemy)>();
        if (!(board.Squares[x, y]?.GetType() == typeof(King))) return moves;
        //Console.WriteLine($"x: {x}, y: {y}");
        if (!movementStates[PlayerColor.White][PieceType.King])
        {
            if (board.Squares[7, 0] != null && board.Squares[7, 0]!.Color == "White"
                                            && friendlyColor == "White" &&
                                            !movementStates[PlayerColor.White][PieceType.LeftRook]
                                            ...
                                            && board.Squares[7, 3] == null)
            {
                ...
                if (ValidateMove((7, 4), (7, 2), board, friendlyColor)) moves.Add((7, 0, false));
            ...
            if (board.Squares[7, 7] != null && board.Squares[7, 7]!.Color == "White"
                                            && friendlyColor == "White" &&
                                            !movementStates[PlayerColor.White][PieceType.RightRook]
                                            && board.Squares[7, 5] == null && board.Squares[7, 6] == null)
            {
                board.Squares[7, 7] = null;
                board.Squares[7, 5] = new Rook("White");

                if (ValidateMove((7, 4), (7, 6), board, friendlyColor)) moves.Add((7, 7, false));
        ...
        if (!movementStates[PlayerColor.Black][PieceType.King])
        {
            if (board.Squares[0, 0] != null && board.Squares[0, 0]!.Color == "Black"
                                            && friendlyColor == "Black" &&
                                            !movementStates[PlayerColor.Black][PieceType.LeftRook]
                                            && board.Squares[0, 1] == null && board.Squares[0, 2] == null &&
                                            board.Squares[0, 3] == null)
            {
                board.Squares[0, 0] = null;
                board.Squares[0, 3] = new Rook("Black");

                if (ValidateMove((0, 4), (0, 2), board, friendlyColor)) moves.Add((0, 0, false));
            ...
            if (board.Squares[0, 7] != null && board.Squares[0, 7]!.Color == "Black"
                                            && friendlyColor == "Black" &&
                                            !movementStates[PlayerColor.Black][PieceType.RightRook]
                                            && board.Squares[0, 5] == null && board.Squares[0, 6] == null)
            {
                board.Squares[0, 7] = null;
                board.Squares[0, 5] = new Rook("Black");

                if (ValidateMove((0, 4), (0, 6), board, friendlyColor)) moves.Add((0, 7, false));
