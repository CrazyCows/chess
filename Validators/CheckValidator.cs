...
{
    public bool Check(IBoard board, string friendlyColor)
    {
        ...
        if ((from ep in enemyPieces
                let piece = squares[ep.x, ep.y]
                select
                    piece.GetMoves((ep.x, ep.y), friendlyPieces, enemyPieces)).Any(moves =>
                moves.Any(move => move.x == kingPos.Value.x && move.y == kingPos.Value.y)))
            return true;
        return false;
    }
