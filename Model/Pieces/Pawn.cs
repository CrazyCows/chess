...
public class Pawn(string Color) : Piece(Color)
{
        ...
        List<(int x, int y)> enemyPieces, List<(int x, int y)> friendlyPieces)
    {
        ...
        if (IsMoveInsideBorder(forwardPos) && !enemyPieces.Contains(forwardPos) && !friendlyPieces.Contains(forwardPos))
        {
            ...
            if (currentPosition.x == startPos && !enemyPieces.Contains(forwardPos2) &&
                !friendlyPieces.Contains(forwardPos2)) moveList.Add((forwardPos2.Item1, forwardPos2.Item2, false));
        }
