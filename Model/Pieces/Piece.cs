...
public abstract class Piece(string color) : IPieces
{
    ...
    public abstract List<(int x, int y, bool IsEnemy)> GetMoves((int x, int y) currentPosition,
    ...
    {
        if (currentPosition.Item1 > 7 || currentPosition.Item1 < 0) return false;
        if (currentPosition.Item2 > 7 || currentPosition.Item2 < 0) return false;
        return true;
    }
